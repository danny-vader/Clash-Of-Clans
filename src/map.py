...
def printMap(V):
    map = np.copy(V.map)
    map_matrix = np.empty(
        (pt.config['dimensions'][0]*2, pt.config['dimensions'][1]*2), dtype=object)
    for i in range(map.shape[0]):
        for j in range(map.shape[1]):
            if (map[i][j].split(':')[0] == pt.BLANK):
                ...
                map_matrix[2*i+4][2*j +
                                  1] = buildingColor(V, map[i][j]) + Fore.BLACK + 'TO'
                map_matrix[2*i+4][2*j +
                                  2] = buildingColor(V, map[i][j]) + Fore.BLACK + 'WN'
                map_matrix[2*i+4][2*j +
                                  3] = buildingColor(V, map[i][j]) + Fore.BLACK + 'HA'
                map_matrix[2*i+4][2*j +
                                  4] = buildingColor(V, map[i][j]) + Fore.BLACK + 'LL'
    ...
    for dr in healers:
        a = 2*dr.position[0]
        b = 2*dr.position[1]
        map_matrix[a][b] = drColor(dr) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = drColor(dr) + Fore.BLACK + 'AL'
        map_matrix[a][b+1] = drColor(dr) + Fore.BLACK + 'ER'
        map_matrix[a+1][b+1] = drColor(dr) + Fore.BLACK + '  '
