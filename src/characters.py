...
class Healer:
    ...
    def move(self, target):
        ...
        r = abs(target.position[0] - self.position[0])
        c = abs(target.position[1] - self.position[1])

        pos = target.position

        if (self.isInAttackradius(r, c) == True):
            self.attack_target(target)
            return
        elif (r == 0):
            if (pos[1] > self.position[1]):
...
def move_healers():
    for hr in healers:
        if (hr.alive == False):
            continue
        closest_troop = search_for_closest_troop(hr.position)
        if (closest_troop == None):
            continue
        hr.move(closest_troop)
